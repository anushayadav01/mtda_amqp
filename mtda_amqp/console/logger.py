...
class ConsoleLogger:
    ...
    def start(self):
        print("In start method of logger file")
        self.rx_alive = True
    ...
    def write(self, data, raw=False):
        try:
            if raw is False:
                ...
                data = bytes(data, "utf-8")
            print(f"The data in the logger part {data}")
            self.console.write(data)
        except IOError as e:
            print(e)
    ...
    def _print(self, data):
        if self.prints is True:
            if self.socket is not None:
                '''
                self.props_result=self.socket.queue_declare(queue='mtda-amqp')
                #self.callback_queue = self.props_result.method.queue
                #self.corr_id=None
                ...
                '''
                self.socket.basic_publish(exchange='',routing_key="mtda-amqp",body=str(data))
